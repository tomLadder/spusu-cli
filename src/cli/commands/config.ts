...
import { loadConfig, saveConfig, resetConfig, getConfigPath } from '../../store/config.ts';
  ...
  config
    ...
    .action(async (options) => {
      ...
      if (options.json) {
        ...
        const sanitized = {
          ...cfg,
          auth: cfg.auth ? { phoneNumber: cfg.auth.phoneNumber, hasSession: !!cfg.auth.sessionId } : undefined,
        };
      ...
      if (cfg.auth?.phoneNumber) {
        console.log(chalk.cyan('Phone:'), cfg.auth.phoneNumber);
      }
      console.log();
